...
public class ImagesControllerTest {
  ...
  public void testGetImages() throws Exception {
    MvcResult result = mockMvc.perform(get("/images").principal(new UsernamePasswordAuthenticationToken(new UserDB("test", "test@email.com", UserRole.USER), null,
  ...
  public void testTagImage() throws Exception {
    ...
    ImageApi image = images.stream().filter(imageApi -> imageApi.getId().equals("13bb0f23-7a13-48bc-bb7d-663243ca7e59")).findFirst().get();
    ...
    mockMvc.perform(put("/images/tag").principal(new UsernamePasswordAuthenticationToken(new UserDB("test", "test@email.com", UserRole.ADMIN), null,
    ...
    result = mockMvc.perform(get("/images").principal(new UsernamePasswordAuthenticationToken(new UserDB("test", "test@email.com", UserRole.USER), null,
                Collections.singleton(new SimpleGrantedAuthority("ROLE_" + UserRole.USER.name()))))
            .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)).andReturn();
    ...
    image = images.stream().filter(imageApi -> imageApi.getId().equals("13bb0f23-7a13-48bc-bb7d-663243ca7e59")).findFirst().get();
    Assertions.assertEquals(1, image.getCoordinates().size());
    Assertions.assertEquals("test@email.com", image.getTaggerUserId());

    mockMvc.perform(put("/images/13bb0f23-7a13-48bc-bb7d-663243ca7e59/validate").principal(new UsernamePasswordAuthenticationToken(new UserDB("test", "test@email.com", UserRole.ADMIN), null,
                Collections.singleton(new SimpleGrantedAuthority("ROLE_" + UserRole.ADMIN.name()))))
            .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(image)))
        .andExpect(status().isNoContent());
